...
- `Ministral` A *much* smaller version of Mistral, which handles the kinds 
  of prompts Alice Assistant uses *very* well for something that can fit 
...
- `Granite` It technically works, and handles simple queries well, and even 
  ...
  can send the harder stuff elsewhere, it has *strong* potential for handling 
  ...
  still get tool calling capability. And it is *fast*. On everything.

## Local models that don't work well, or at all, with Alice Assistant
None so far.

## Cloud models that work well with Alice Assistant
- `GLM-5` My favorite cloud model for this assistant. This one handles the 
  default personality very well. It also handles complex instructions well, 
  and while I haven't gotten the broken tool calls Mistral occasionally spits 
...
- `Mistral Large 670b` This one handles the default personality *flawlessly*, 
  if maybe making it a little *too* intense. It pukes out broken tool calls 
...
- `Minimax m2.5` It *usually* works, but sometimes flat-out refuses to play 
  ...
  attributes tool result messages *to the user* which results in some weird 
  responses. Unfortunate, because when it does work, the output is *spectacularly* 
  ...
  a good alternative model for agent tasks that are better off *without* much 
...
- `Gemma 4` I'm rather excited to try this one. I've already "faked it" by 
  sending the personality and query prompts to Gemini, and aside from being 
  *way* more verbose than I'd like, it handled everything else rather well. 
  Since Gemma 4 is rather close to what the latest Gemini can do, I'm 
