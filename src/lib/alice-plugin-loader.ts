...
export async function loadPlugins() {
  ...
  const userPluginConfigPath = path.join(
    aliceDir,
    'plugin-settings',
    'enabled-plugins.json'
  );
  const userPluginConfigExists = await exists(userPluginConfigPath);
  const rawEnabledPlugins = userPluginConfigExists
    ? (JSON.parse(
        await readFile(userPluginConfigPath, 'utf-8')
      ) as Partial<EnabledPluginsConfig>)
  ...
  const userEnabledPlugins: EnabledPluginsConfig = {
    ...
    user: {
      enableUserPlugins:
        rawEnabledPlugins.user?.enableUserPlugins ??
        defaultEnabledPlugins.user.enableUserPlugins,
      plugins:
        rawEnabledPlugins.user?.plugins ?? defaultEnabledPlugins.user.plugins,
    },
  };
  ...
  const builtInPlugins: BuiltInPluginDefinition[] = JSON.parse(
    await readFile(
      path.join(builtInPluginsPath, 'system-plugins.json'),
      'utf-8'
    )
  );

  const enabledBuiltInPlugins = builtInPlugins
    .map(plugin => {
      if (plugin.required && !userEnabledPlugins.system[plugin.id]) {
        throw new Error(
          `Built-in plugin ${plugin.name} (${plugin.id}) is required but not enabled. Please enable it in your enabled-plugins.json to continue.`
        );
      }
  ...
  const enabledUserPlugins = userEnabledPlugins.user.enableUserPlugins
    ? Object.keys(userEnabledPlugins.user.plugins).filter(
        pluginId => userEnabledPlugins.user.plugins[pluginId]
      )
    : [];
  const allEnabledPluginIds = [
    ...enabledBuiltInPlugins.map(p => p.id),
    ...enabledUserPlugins,
  ];

  // Check for unknown plugins in the enabled list.
  const unknownSystemPlugins = Object.keys(userEnabledPlugins.system).filter(
    pluginId => !builtInPlugins.find(p => p.id === pluginId)
  );

  if (unknownSystemPlugins.length > 0) {
    throw new Error(
      `Unknown system plugins found in enabled-plugins.json: ${unknownSystemPlugins.join(', ')}. Please remove these entries to continue.`
    );
  }
  ...
  const existingUserPlugins = await Promise.all(
    existingUserPluginDirs.map(async dir => {
  ...
  const enabledPlugins = await Promise.all(
    allEnabledPluginIds.map(async pluginId => {
  ...
  enabledPlugins.forEach(plugin => {
    ...
    const isBuiltInPlugin = !!builtInPlugins.find(
      p => p.id === plugin.pluginMetadata.id
    );
    const isUsingPrivilegedFlags =
      plugin.pluginMetadata.required || plugin.pluginMetadata.builtInCategory;
    if (
      isUsingPrivilegedFlags &&
      !builtInPlugins.find(p => p.id === plugin.pluginMetadata.id)
    ) {
      throw new Error(
        `Plugin ${plugin.pluginMetadata.id} is claiming to be a built-in or required ` +
    ...
    if (!isBuiltInPlugin && plugin.pluginMetadata.version === 'LATEST') {
      throw new Error(
        `Plugin ${plugin.pluginMetadata.id} is using the reserved version string LATEST. Only built-in shipped plugins may do this. Disable ${plugin.pluginMetadata.id} to fix your assistant, or replace LATEST with a real semver version if you are developing this plugin.`
      );
    }

    plugin.pluginMetadata.dependencies?.forEach(dependency => {
      const found = enabledPlugins.find(
      ...
      if (isBuiltInPlugin) {
        ...
        throw new Error(
          `Plugin ${plugin.pluginMetadata.id} depends on ${dependency.id} using the reserved version string LATEST. Only built-in shipped plugins may use LATEST dependency ranges. Disable ${plugin.pluginMetadata.id} to fix your assistant, or replace LATEST with a real semver range if you are developing this plugin.`
        );
      }

      if (!satisfies(found.pluginMetadata.version, dependency.version)) {
        throw new Error(
          `Plugin ${plugin.pluginMetadata.id} has an incompatible version of ` +
    ...
    function visit(pluginId: string) {
      if (visited.has(pluginId)) {
        throw new Error(
          `Dependency cycle detected involving plugin ${pluginId}. To fix your assistant immediately, disable one of these plugins: ${[...visited].join(', ')}. If you are currently developing any of these plugins, please check your dependencies for cycles and remove them to continue.`
        );
      }
