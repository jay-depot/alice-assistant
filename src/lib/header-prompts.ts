...
const headerPrompts: DynamicPrompt[] = [
  scenarioHeaderPrompt,
  toolsHeaderPrompt,
];

export async function getHeaderPrompts(
  context: DynamicPromptContext
): Promise<string[]> {
  return processDynamicPrompts(
    context,
    headerPrompts.filter(
      prompt =>
        context.toolCallsAllowed !== false || prompt.name !== 'toolsHeader'
    )
  );
}
