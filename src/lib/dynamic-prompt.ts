...
export type DynamicPrompt = {
  ...
  getPrompt: (context: DynamicPromptContext) => Promise<string | false> | string | false;
};

export async function processDynamicPrompts(context: DynamicPromptContext, dynamicPrompts: DynamicPrompt[]): Promise<string[]> {
  const applicablePrompts = await Promise.all(dynamicPrompts.map(async (prompt) => {
    const promptText = await prompt.getPrompt(context);
    return { ...prompt, promptText };
  }))
    .then(prompts => prompts.filter(prompt => prompt.promptText !== false))
    .then(prompts => prompts.sort((a, b) => {
