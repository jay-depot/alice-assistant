...
export function buildFallbackChatNotification(
...
export function buildNotificationChatTitle(
  notification: NotificationPayload
): string {
  ...
  const normalizedTitle =
    preferredTitle.replace(/\s+/g, ' ').trim() || 'Needs Attention';
  const clippedTitle =
    normalizedTitle.length > 48
...
export async function renderChatNotificationInVoice(
  ...
  const response = await Conversation.sendDirectRequest([
    {
      role: 'system',
      content: await renderPersonalityPrompt({
    ...
    },
    {
      role: 'system',
      content:
        `You are ${config.assistantName}. ${scenarioInstruction} ` +
