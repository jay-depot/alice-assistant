...
const parameters = Type.Object({
  ...
  maxBytes: Type.Optional(Type.Number({ description: 'Maximum bytes to read (default: 65536, max: 1048576)' })),
  ...
  execute: async (args: Static<typeof parameters>) => {
    ...
    if (allowedRoots.length === 0) {
      return JSON.stringify({
        error: 'No allowed root directories configured for readUserTextFile tool.'
      });
    }
    ...
    const maxAllowedSize = config.toolSettings.readUserTextFile?.maxFileSizeBytes || 10485760; // 10MB default
    ...
    try {
      ...
      return JSON.stringify({
        ...
        content: truncated,
        message: wasComplete ? 'Complete file contents' : 'Partial contents (truncated)'
      });
    } catch (err) {
      return JSON.stringify({
