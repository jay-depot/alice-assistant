...
const NewsToolParametersSchema = Type.Object({
  query: Type.String({
    description:
      'The news topic to search for. This can be as broad or specific as you like, but should be focused on a particular topic or event.',
  }),
});
export type NewsToolParametersSchema = Type.Static<
  typeof NewsToolParametersSchema
>;
...
const newsBrokerPlugin: AlicePlugin = {
  ...
  async registerPlugin(pluginInterface) {
    ...
    const newsProviderCallbacks: Record<
      string,
      (query: string) => Promise<NewsItem[]>
    > = {};

    const requestNewsData = async (
      query: string
      ...
      const cleanedQuery = query
        ...
        .join(' ');
      await Promise.all(
        Object.entries(newsProviderCallbacks).map(async ([name, callback]) => {
    ...
    plugin.registerTool({
      ...
      description:
        'Gets news data related to a specific query from any plugin that offers it ' +
        ...
        const formattedResults = Object.entries(newsData)
          .map(([provider, items]) => {
            const formattedItems = items
              .map(
                item =>
                  `- ${item.headline} (${item.source}, ${item.age} ago)\n${item.preview ? `  ${item.preview}\n` : ''}  URL: ${item.url}\n`
              )
              .join('\n');
