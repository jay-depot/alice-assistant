...
const CreateReminderToolParametersSchema = Type.Object({
  reminderMessage: Type.String({ description: 'The message to be included in the reminder.' }),
  scheduledFor: Type.String({ description: 'The date and time when the reminder should be delivered, in ISO 8601 format.' }),
});

const UpdateReminderToolParametersSchema = Type.Object({
  id: Type.String({ description: 'The ID of the reminder to update.' }),
  updatedDetails: Type.Partial(Type.Object({
    reminderMessage: Type.String({ description: 'The updated message for the reminder.' }),
    scheduledFor: Type.String({ description: 'The updated date and time for the reminder, in ISO 8601 format.' }),
  })),
});
...
export type CreateReminderToolParameters = Type.Static<typeof CreateReminderToolParametersSchema>;
export type UpdateReminderToolParameters = Type.Static<typeof UpdateReminderToolParametersSchema>;
export type CancelReminderToolParameters = Type.Static<typeof CancelReminderToolParametersSchema>;

const remindMePlugin: AlicePlugin = {
  pluginMetadata: {
    id: 'remind-me',
    name: 'Remind Me Plugin',
    description: 'A plugin that allows the assistant to directly create reminders on behalf ' +
  ...
  registerPlugin: async (pluginInterface) => {
    ...
    const { createNewReminder, updateReminder, deleteReminder } = plugin.request('reminders-broker')!;

    plugin.registerTool({
      name: 'createReminder',
      description: 'Creates a new one-shot reminder with the given message and schedule.',
    ...
    plugin.registerTool({
      name: 'updateReminder',
      description: 'Updates an existing one-shot reminder with the given ID and new details.',
      ...
      execute: async (params: UpdateReminderToolParameters) => {
        const newDate = params.updatedDetails.scheduledFor ? new Date(params.updatedDetails.scheduledFor) : undefined;
