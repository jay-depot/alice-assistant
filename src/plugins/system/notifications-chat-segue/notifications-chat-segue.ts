...
const MarkNotificationsDeliveredParameters = Type.Object({
  notificationIds: Type.Array(Type.String(), {
    description: 'The IDs of pending notifications that have already been clearly delivered to the user in conversation.',
  }),
});
  ...
  async registerPlugin(pluginInterface) {
    ...
    await registerNotificationSink('notifications-chat-segue', {
      sendNotification: async (notification) => {
        ...
        const notificationEntry = em.create(NotificationsChatSegueNotification, {
    ...
    plugin.registerHeaderSystemPrompt({
      ...
      getPrompt: async (context) => {
        ...
        const orm = await awaitForOrm;
        const em = orm.em.fork();
        const pendingNotifications = await em.find(NotificationsChatSegueNotification, {
          status: 'pending',
        }, {
    ...
    plugin.registerTool({
      ...
      availableFor: ['chat'],
      description: 'Marks pending segue notifications as delivered after you have clearly mentioned them to the user in chat.',
      parameters: MarkNotificationsDeliveredParameters,
      systemPromptFragment: 'Call markNotificationsDelivered only after you have already worked one or more pending notifications into the conversation clearly enough that the user has effectively received them.',
      toolResultPromptIntro: 'You have updated the delivery state of pending chat notifications.',
      ...
      execute: async (args) => {
        ...
        const orm = await awaitForOrm;
        const em = orm.em.fork();
        const notifications = await em.find(NotificationsChatSegueNotification, {
          id: { $in: parsedIds },
          status: 'pending',
        });
