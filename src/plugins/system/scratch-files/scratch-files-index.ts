...
import { exists, readdir, readFile, stat, writeFile } from '../../../lib/node/fs-promised.js';
...
export async function freshenScratchFilesIndex(config: ScratchFilesPluginConfigSchema) {
  ...
  const indexLastModified = (await exists(indexFilePath)) ? (await stat(indexFilePath)).mtime : null;
  const index = (await exists(indexFilePath)) ? JSON.parse(await readFile(indexFilePath, 'utf-8')) : {};
  const files = await readdir(scratchDirectory);
  for (const file of files) {
    const isHidden = file.startsWith('.');
    const isAllowedType = config.allowedFileTypes.includes(path.extname(file).substring(1));
    if (isHidden || !isAllowedType) {
      console.log(`Skipping file ${file} (hidden: ${isHidden}, allowed type: ${isAllowedType})`);
      continue
    };
    ...
    if (stats.isFile()) {
      ...
      if (!index[file] || !indexLastModified || lastModified > indexLastModified) {
...
export async function reindexScratchFiles(config: ScratchFilesPluginConfigSchema) {
  const scratchDirectory = simpleExpandTilde(config.scratchDirectory);
  const indexFilePath = path.join(scratchDirectory, '.index');
...
function summarizeFileContent(file: string, content: string) {
  const summaryRequest: Message[] = [
    {
      role: 'system',
      content: 'You are a helpful assistant that summarizes the content of files. ' +
