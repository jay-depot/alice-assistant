...
export const VoicePluginConfigSchema = Type.Object({
  ...
  deferredSessionCloseDelayMs: Type.Optional(Type.Number({
    description: 'How long to wait before finalizing an expired voice conversation in the background after a fresh one starts.',
  })),
  ...
  maxCaptureSeconds: Type.Optional(Type.Number({
    description: 'Hard maximum recording length for a single captured voice turn.',
  })),
  trailingSilenceMs: Type.Optional(Type.Number({
    description: 'How much consecutive low-energy audio after speech should end the current capture.',
  })),
  speechThreshold: Type.Optional(Type.Number({
    description: 'Normalized RMS threshold used to decide whether an audio chunk counts as speech for first-pass silence detection.',
  })),
  backgroundNoiseSampleSeconds: Type.Optional(Type.Number({
    description: 'How long to sample ambient room noise after wake-word detection and before the listening sound plays.',
  })),
  prerollMs: Type.Optional(Type.Number({
    description: 'How much recent audio to retain before speech starts so the beginning of an utterance is not clipped.',
  })),
