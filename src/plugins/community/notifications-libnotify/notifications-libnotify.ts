...
const notificationsLibnotifyPlugin: AlicePlugin = {
  ...
  async registerPlugin(pluginInterface) {
    ...
    if (process.platform !== 'linux') {
      throw new Error(
        'Plugin notifications-libnotify requires Linux with notify-send available. Disable notifications-libnotify to continue.'
      );
    }

    if (!hasNotifySend()) {
      throw new Error(
        'Plugin notifications-libnotify requires the notify-send command. Install libnotify-bin or disable notifications-libnotify to continue.'
      );
    }
