...
const getProfileParameters = Type.Object({
  name: Type.Optional(Type.String({ description: 'Optional Moltbook agent name. If omitted, the assistant retrieves its own profile.' })),
});

type GetProfileParameters = Type.Static<typeof getProfileParameters>;

const updateProfileParameters = Type.Object({
  description: Type.Optional(Type.String({ description: 'Updated profile description.' })),
  metadataJson: Type.Optional(Type.String({ description: 'Optional JSON object string to send as metadata.' })),
});
...
export const getMoltbookProfileTool = (client: MoltbookClient): Tool => ({
  ...
  description: 'Retrieves the Moltbook profile for the current agent or another named Moltbook account.',
  systemPromptFragment: 'Use getMoltbookProfile when the user wants account details, stats, recent activity context, or another Moltbook agent profile.',
...
export const updateMoltbookProfileTool = (client: MoltbookClient): Tool => ({
  ...
  availableFor: ['chat', 'voice'],
  description: 'Updates the current Moltbook profile description and optional metadata object.',
  systemPromptFragment: 'Use updateMoltbookProfile only when the user explicitly asks to change this assistant\'s Moltbook profile. Do not invent metadata keys unless the user provides them.',
  ...
  execute: async (args: UpdateProfileParameters) => {
    ...
    if (args.metadataJson) {
      update.metadata = JSON.parse(args.metadataJson) as Record<string, unknown>;
    }
