...
const markNotificationsParameters = Type.Object({
  mode: Type.Optional(
    Type.Union([Type.Literal('all'), Type.Literal('post')], { default: 'all' })
  ),
  postId: Type.Optional(
    Type.String({ description: 'Required when mode is post.' })
  ),
});

type MarkNotificationsParameters = Type.Static<
...
export const markMoltbookNotificationsReadTool = (
  ...
  execute: async (args: MarkNotificationsParameters) => {
    if (args.mode === 'post') {
      ...
      const message =
        typeof result.message === 'string'
          ? result.message
          : 'Notifications for that post were marked as read.';
    ...
    const message =
      typeof result.message === 'string'
        ? result.message
        : 'All Moltbook notifications were marked as read.';
    return message;
  },
});
