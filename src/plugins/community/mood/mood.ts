...
const moodPlugin: AlicePlugin = {
  ...
  async registerPlugin(pluginInterface) {
    ...
    const currentMood: { mood: string; reason: string } = {
      mood: 'neutral',
      reason: 'Default on assistant startup',
    };
    ...
    if (fs.existsSync(toolConfigPath)) {
      try {
        const lastMoodData = JSON.parse(
          fs.readFileSync(path.join(toolConfigPath, 'last-mood.json'), 'utf-8')
        );
    ...
    function saveMood(mood: string, reason: string) {
      ...
      fs.writeFileSync(
        path.join(toolConfigPath, 'last-mood.json'),
    ...
    plugin.registerTool({
      ...
      description:
        `Sets the assistant's mood. The mood is a string that describes the tone of the ` +
      ...
      systemPromptFragment:
        `The assistant has a mood, which is a string that describes tone and delivery of ` +
        `the assistant's responses. The mood can be set by calling the mood.set tool with the new mood and a ` +
