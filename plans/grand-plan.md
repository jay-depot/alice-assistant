...
> **Status (2026-04-14): PARTIALLY COMPLETE.** Task assistant system (`TaskAssistants` in `src/lib/task-assistant.ts`) is fully implemented with `createTaskAssistantToolPair` factory, conversation type registration, start/complete lifecycle, and webhook-based events. The `brainstorm` plugin (`src/plugins/community/brainstorm/brainstorm.ts`) is implemented as a working task assistant with start/complete tools and a dedicated `brainstorm` conversation type. Session-linked agents (`AgentSystem` in `src/lib/agent-system.ts`) are also implemented with `reportProgress`/`returnResult` tools. What's NOT yet done: additional task assistant examples beyond brainstorm (e.g., "planning assistant", "writing assistant", "troubleshooting assistant"), and the completion tool UX in the web UI for ending task assistant sessions.
...
- **Status (2026-04-14): PARTIALLY COMPLETE.** Session-linked agents are implemented (`AgentSystem` in `src/lib/agent-system.ts`, `agents` plugin in `src/plugins/system/agents/`). This covers: agent registration, lifecycle management, `reportProgress`/`returnResult` framework tools, and cancellation on session end. The `deep-dive` community plugin exists as a session-linked agent example. What's NOT yet done: scheduled-session agents, independent agents, agent management UI in the web interface, the full lifecycle state model described in `plans/agent-dispatching.md` (only `running`/`cancelled`/`erroring`/`completed` are implemented — no `hatching`, `handoff pending`, `stuck`, `sleeping`, `paused`, etc.), and the metadata/registration rules are not enforced beyond basic ID uniqueness. See `plans/agent-dispatching.md` for remaining work.
- see `plans/agent-dispatching.md` for the detailed plan.
...
- **Status (2026-04-14): COMPLETE.** Show tool calls in the output — fully implemented with `ToolCallEvents` dispatch system, `callBatchId` batching in `conversation.ts`, WebSocket push channel, `ToolCallIndicator`/`ToolCallBatch` React components, `useToolCallEvents` hook, `tool_call` message kind in DB schema, and batch grouping in the message list. See `plans/tool-call-visibility.md` (all 5 phases implemented).
- Split mood plugin into mood tracking, and mood displays. **NOT STARTED.** Make a few other mood display plugins. These generally shouldn't conflict, so enabling more than one should be allowed.
